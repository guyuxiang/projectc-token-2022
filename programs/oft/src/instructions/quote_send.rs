...
pub fn compute_fee_and_adjust_amount(
    amount_ld: u64,
    oft_store: &OFTStore,
    token_mint: &InterfaceAccount<Token2022MintCompat>,
    fee_bps: Option<u16>,
) -> Result<(u64, u64, u64)> {
    require!(oft_store.oft_type == OFTType::Native, OFTError::Unauthorized);
    require!(fee_bps.unwrap_or(0) == 0, OFTError::InvalidFee);
    let amount_sent_ld = oft_store.remove_dust(amount_ld);
    let amount_received_ld = amount_sent_ld;
    Ok((amount_sent_ld, amount_received_ld, 0))
}
