...
pub struct LzReceive<'info> {
    ...
    pub token_dest: Box<InterfaceAccount<'info, Token2022AccountCompat>>,
    #[account(
        mut,
        address = oft_store.token_mint,
        mint::token_program = token_program
    )]
    pub token_mint: Box<InterfaceAccount<'info, Token2022MintCompat>>,
...
impl LzReceive<'_> {
    pub fn apply(ctx: &mut Context<LzReceive>, params: &LzReceiveParams) -> Result<()> {
        ...
        let oft_store_seed = ctx.accounts.token_mint.key();
        ...
        if let Some(mint_authority) = &ctx.accounts.mint_authority {
            // Native type
            // mint
