...
import { DebugLogger, KnownWarnings, createSolanaConnectionFactory } from './wireSupport'
...
type DeriveConnectionParams = boolean | { readOnly?: boolean; noopSigner?: PublicKey }
...
export const deriveConnection = async (eid: EndpointId, params: DeriveConnectionParams = false) => {
    ...
    return { connection, umi, umiWalletKeyPair, umiWalletSigner }
}

export const useWeb3Js = async () => ({ web3JsKeypair: await getSolanaKeypair() })
...
export const deriveKeys = (programIdStr: string) => {
    ...
    return { programId, lockBox, escrowPK, oftStorePda, eddsa }
}
...
export const saveSolanaDeployment = (
    ...
    writeFileSync(
        `${outputDir}/OFT.json`,
        JSON.stringify({ programId, mint, mintAuthority, escrow, oftStore }, null, 4)
    )
...
export const getSolanaDeployment = (
    eid: EndpointId
): { programId: string; mint: string; mintAuthority: string; escrow: string; oftStore: string } => {
    if (!eid) throw new Error('eid is required')
    const outputDir = path.join('deployments', endpointIdToNetwork(eid))
    const filePath = path.join(outputDir, 'OFT.json')
...
const getAddressLookupTable = async (_lookupTableAddress: string | PublicKey, connection: Connection, umi: Umi) => {
    ...
    if (!addressLookupTableInput) throw new Error(`No address lookup table found for ${lookupTableAddress}`)
    const { value: lookupTableAccount } = await connection.getAddressLookupTable(toWeb3JsPublicKey(lookupTableAddress))
    if (!lookupTableAccount) throw new Error(`No address lookup table account found for ${lookupTableAddress}`)
...
const TransactionCuEstimates: Record<TransactionType, number> = {
    [TransactionType.CreateToken]: 125_000,
    [TransactionType.CreateMultisig]: 5_000,
    [TransactionType.InitOft]: 70_000,
    [TransactionType.SetAuthority]: 8_000,
    [TransactionType.InitConfig]: 42_000,
    [TransactionType.SendOFT]: 230_000,
}
...
export const getComputeUnitPriceAndLimit = async (
    ...
    try {
        computeUnits = await backOff(
            ...
            { maxDelay: 10000, numOfAttempts: 3 }
        )
    } catch (e) {
    ...
    if (!computeUnits) throw new Error('Unable to compute units')
    return { computeUnitPrice, computeUnits }
}
...
export const addComputeUnitInstructions = async (
    ...
    const computeUnitLimitScaleFactor = 1.1
