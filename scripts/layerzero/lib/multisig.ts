...
import { TransactionType, addComputeUnitInstructions, getExplorerTxLink } from './solanaRuntime'
...
export const checkMultisigSigners = async (
    ...
    if (numRequiredSigners !== 1) throw new Error('Multisig account must have 1 required signer')

    const signers: PublicKey[] = []
    const signerOffset = 3
    const signerSize = 32
    for (let i = 0; i < numTotalSigners; i++) {
        ...
        if (!signerPublicKey.equals(PublicKey.default)) signers.push(signerPublicKey)
    }
