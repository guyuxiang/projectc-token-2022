...
export function parseDecimalToUnits(amount: string, decimals: number): bigint {
    ...
    const fracUnits = BigInt(fraction.padEnd(decimals, '0').slice(0, decimals))
...
export async function validateSigningAuthority(
    ...
    if (signingAuthority !== admin) {
        warnings.push(
            `Signing authority (${signingAuthority}) is not the admin (${admin}). Use the correct keypair or supply the correct value for --multisig-key if the admin is a Squads Vault.`
        )
    }
    if (delegate && signingAuthority !== delegate) {
        warnings.push(
            `Signing authority (${signingAuthority}) is not the delegate (${delegate}). Use the correct keypair or supply the correct value for --multisig-key if the delegate is a Squads Vault.`
        )
    }
