...
import { deriveConnection, getSolanaDeployment, useWeb3Js } from '../../scripts/layerzero/lib/solanaRuntime'
import { findSolanaEndpointIdInGraph, validateSigningAuthority } from '../../scripts/layerzero/lib/solanaUtils'
import {
    createSdkFactory,
    createSolanaConnectionFactory,
    createSolanaSignerFactory,
    getSolanaUlnConfigPDAs,
} from '../../scripts/layerzero/lib/wireSupport'
...
const publicKeyType: CLIArgumentType<PublicKey> = {
    name: 'keyPair',
...
task(TASK_LZ_OAPP_WIRE)
    ...
    .setAction(async (args: Args, hre, runSuper) => {
        ...
        subtask(SUBTASK_LZ_SIGN_AND_SEND, 'Sign OFT transactions', (args: SignAndSendTaskArgs, _hre, runSuper) =>
            runSuper({
                ...args,
                createSigner: async (eid) => {
                    if (endpointIdToChainType(eid) === ChainType.SOLANA) {
                        return solanaSignerFactory(eid)
                    }
                    return args.createSigner(eid)
                },
            })
        )
